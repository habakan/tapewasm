...
//! series a hand-written host stub used here was accurate to 2e-9 — which is
//! what a gradient through `student_t` was worth. Against a reference
//! implementation at the same point it now agrees to 3e-14 rather than 8e-9.
//!
//! These mirror `tapewasm_autodiff`'s versions operation for operation, so the
...
/// The same value `tapewasm_autodiff` uses.
...
mod tests {
    ...
    /// Against the same algorithm in `tapewasm_autodiff`, which is what the
    ...
    fn lgamma_matches_the_recorded_tape() {
        check("lgamma", tapewasm_autodiff::lgamma);
    }
