//! Emit a recorded tape as a standalone wasm module computing the value and
//! every partial derivative in one call.
//!
//! [`compile_tape`] is the whole entry point. It reads the tape and nothing
//! else, so what recorded the tape — a model language, a graph walker in
//! another process, a hand-written stream — makes no difference here.
//!
//! Written as wasm binary directly via `wasm-encoder`: no WAT step, and
//! nothing for the browser to parse but the module.
//!
...
pub enum CodegenError {
    ...
    Internal(String),
    #[error(
        "`{op}` is not supported yet — the emitter has no instruction \
         sequence for it. The tape's own reverse pass computes every op it \
         can record, so a caller that can replay the tape still has a path."
    )]
...
/// carries this as the immutable i32 global `tapewasm_abi_version`, and a host
...
pub struct Compiled {
    ...
    /// the immutable i32 global `tapewasm_layout_id`. Calling one model's
...
fn emit(tape: &Tape, n_params: usize, root: u32, reroll: Reroll) -> (Vec<u8>, Vec<f64>, u32) {
    ...
    // Memory is imported from "tapewasm" — shared with the host wasm.
    imports.import(
        "tapewasm",
        "memory",
