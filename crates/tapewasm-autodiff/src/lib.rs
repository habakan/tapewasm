...
//! state, so a caller records one per compilation. It is where a front end
//! writes the expression it wants differentiated; `tapewasm-codegen` reads it
//! back and emits a wasm module, which is what runs per draw.
...
pub fn trigamma(x: f64) -> f64 {
    ...
    let series = 1.0 / 6.0
        - r2 * (1.0 / 30.0
            - r2 * (1.0 / 42.0 - r2 * (1.0 / 30.0 - r2 * (5.0 / 66.0 - r2 * (691.0 / 2730.0)))));
